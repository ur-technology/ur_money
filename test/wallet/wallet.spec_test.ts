...
describe('WalletModel Service', () => {
  ...
  it('Validating amount', () => {
    WalletModel.generate(config.seed, config.salt).then((data) => {
      ...
      expect(WalletModel.validateAmount(config.fromAddress, null)).toBeFalsy();

      expect(WalletModel.validateAmount(config.fromAddress, config.amountExisted + 200)).toBeFalsy();

      expect(WalletModel.validateAmount(config.fromAddress, 0)).toBeFalsy();

      expect(WalletModel.validateAmount(config.fromAddress, config.amountSent)).toBeTruthy();
    });
  });
  ...
  it('Transaction', (done) => {
    WalletModel.generate(config.seed, config.salt).then((data) => {
      ...
      let first = false;
      wallet.sendRawTransaction(config.toAddress, config.amountExisted + 200).then(function() {
      }, (error) => {
      ...
      let second = false;
      wallet.sendRawTransaction(config.toAddress, config.amountSent).then(function() {
      }, (error) => {
